...
export class ReportController {
    ...
    public generateController = async (req: Request, res: Response) => {
        try {
            ...
            if (typeof simi_state !== 'undefined' || typeof state !== 'undefined' || typeof from !== 'undefined') {
                query += " WHERE";
            }

            if (typeof simi_state !== 'undefined') {

                query += ` inv_simi_state = ${simi_state}`;

                if (typeof state !== 'undefined' || typeof from !== 'undefined') {
            ...
            if (typeof from !== 'undefined') {
                ...
                if (typeof from !== 'undefined' && typeof to !== 'undefined') {
                    ...
                    query += ` inv_created_at >= "${from} 00:00:00" AND inv_created_at <= "${to} 23:59:59"`;
                } else {
                    
                    query += ` inv_created_at >= "${from} 00:00:00"`;
                }
            }
            
            // ./Set query conditions

            query += " ORDER BY `Invoice`.`inv_reference` ASC";
            
            const [ DATA, META_DATA ] = await LOCAL_SEQUELIZE.query(query);
            
            if (DATA.length === 0) {
                res.status(404).send({ status: 404, message: "NO_MATCHES_FOUND" });
            } else {
